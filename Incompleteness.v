...
Section GoedelsIncompletenessThm.
...
Section Goedel_Incompleteness1.
  ...
  Variable proof : nat -> nat -> Prop.
  Hypothesis proofH : forall p, (T |- p) <-> (exists n, proof n ⌜p⌝).  

  Variable Prov : LP.
  Hypothesis arProv : Ar Prov = 2.
  Hypothesis ProvH : forall n m, proof n m <-> (T |- Prov.(`n, `m)). 
  Definition Pr : LP := ext Prov.
  (* Definition Omega_ConT := forall p, (T |- ext p) -> exists n, (T |- p.(`n)).*)
  Definition Omega_ConT := forall p, (forall n, T |- p.(`n)) -> ~ (T |- ext (~~ p)).
  Hypothesis OmConext : Omega_ConT -> forall p, (T |- ext p) -> exists n, (T |- p.(`n)).


  Lemma Pr_sbs : forall n, Pr.(`n) = ext (Prov.('0, `n)).
  Proof.
    unfold ext.
    intros.
    simpl.
  ...
  Proof.
    ...
    - intros.
      ...
      rewrite <-ProvH.
      auto.
    - rewrite -> Pr_sbs. intro.
      rewrite -> proofH.
      apply OmConext with (p := Prov.('0, `⌜p⌝)) in H.
      destruct H as [n].
      exists n.
      rewrite -> ProvH.
      rewrite <- Prov_sbs.
  ...
  Definition ConGT : LP := ~~ Pr.(`⌜~~ !0 == !0⌝).
  Definition G : LP := fixpoint (~~ Pr).

  Lemma Gfixpoint : T |- G <--> ~~ Pr.(`⌜G⌝).
  Proof.
    assert (Ar (~~ Pr) = 1).
    simpl.
    auto.
